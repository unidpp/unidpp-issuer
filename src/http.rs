...
//! the house pattern (ported from `unidpp-registry`'s test support and
//! `unidpp-resolver`'s `httpc`): no reqwest/hyper-client/TLS dependency
//! in either the service or its tests. `http://` only; the issuer talks
//! to sibling services on loopback or inside the same trust boundary,
//! and production TLS terminates at a fronting proxy.
//!
//! Shared by two consumers:
//! - [`crate::registry::RegistryClient`] (forwarding admin
//!   registrations to a `unidpp-registry` instance), and
//! - the integration tests (driving this service itself).
...
/// Convenience wrapper for JSON requests (the registry forwarder and
/// the tests). Returns the raw response.
...
mod tests {
    ...
    #[test]
    fn url_parsing() {
        let u = Url::parse("http://127.0.0.1:8090/profiles?class=x").unwrap();
        assert_eq!(u.host, "127.0.0.1");
        assert_eq!(u.port, 8090);
        assert_eq!(u.path_and_query, "/profiles?class=x");
    ...
    fn query_component_encoding() {
        assert_eq!(
            Url::encode_query_component("urn:unidpp:profile:eu x"),
            "urn%3Aunidpp%3Aprofile%3Aeu%20x"
        );
